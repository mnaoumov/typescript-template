...
const DIVERGENCE_EXCEPTIONS: Readonly<Record<string, string>> = {
  'scripts/helpers/eslint.ts': 'This repo has taken the exit classification that tells a CRASHED linter apart from a red codebase, and `obsidian-test-mocks` has not yet. Handing the exit code to `execFromRoot` and letting it throw reports a rule that threw, a broken config and a Windows access violation the same way it reports real findings, so the reader goes hunting for a defect in source code that was never judged. It has also taken the `--max-warnings 0` that makes the other end of that classification true: ESLint exits `0` for a run that reported warnings and no errors, so without the flag a standing warning reads to the gate as a clean tree and can stand for ever. The sync therefore runs the other way here: the peer takes this, and this entry is deleted when it has - which the gate itself demands, because it fails on an exception still claiming a divergence that has gone away.',
  'scripts/helpers/exec.ts':
    'Same direction, same reason, five fixes the peer has yet to take: `executeBatches` returned a hard-coded `exitCode: 0` with an empty `stderr` to any caller that asked for details, whatever its batches did; the Windows command-line ceiling budgeted cmd.exe\'s whole 8191 for the inner command and so overshot by the `<ComSpec> /d /s /c ""` wrapper, which made every full-sized batch fail with `The command line is too long.`; the aggregate\'s streams now drop the silent batches rather than joining them into blank lines; and the budget is spent on the length cmd.exe MEASURES rather than the length the string holds, since the `^` per cmd meta character goes in after the fact, which made a meta-heavy batch fail with that same message from the other direction; and every argument is quoted for the shell that reads it - batched arguments used to be joined in raw, and static ones took the Windows quoting everywhere, so off Windows `sh` parsed a batched `(` as syntax and expanded a `*` or `$` itself. Deleted when the peer has converged.',
