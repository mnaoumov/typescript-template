...
- **Those helpers are PEER COPIES, shared byte-for-byte with `obsidian-test-mocks` and `obsidian-typings-crawler`** — ten of the eleven files there hash identically in all three checkouts (measured 2026-09-20), and most of them descend from `obsidian-dev-utils/src/script-utils/`. A change to one is a change to all three, so sync it across rather than hand-editing a single copy. Three things to know before moving bytes in either direction:
  ...
  2. **`git-content.ts` is the one file that differs, and the difference is entirely prose.** All three copies carry identical code; the `@file` header and the `isMissingPath` comment are worded three different ways, and this repo's is the current one on what happens outside a repository (it is thrown, not reported as an absent blob). Do NOT "converge" this file toward either sibling — the sync runs the other way.
  3. **Nothing gates the roster, so a copy can fall behind unobserved, and five of them had.** `npm run check:vendored-eslint-rules` walks for the names `obsidian-dev-utils` publishes under `src/script-utils/linters/eslint-rules/`, so it can never see these files; `obsidian-test-mocks` carries a `check:copy-sync` roster gate, but over its docs pipeline rather than over these helpers. A roster gate of the same shape is the settled answer for this repo and is not built yet — until it is, a sync is a hand-diff against both siblings.
